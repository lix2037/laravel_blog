...
function showNoFormDialog(mode, msg, t, sd_width) {
	var sd_width = sd_width ? sd_width : 400;
	var mode = in_array(mode, ['confirm', 'window', 'info', 'loading']) ? mode : 'alert';
	var t = t ? t : "提示信息";
	var msg = msg ? msg : "";
...
function showNoFormWindow(title, the_url, sd_width) {
	...
	$.ajax({
		...
		data		: "isajax=1",
		success		: function(data){
			showNoFormDialog("window", data, title, sd_width);
		},
		error		: function(data){
			showNoFormDialog("alert", "读取数据失败");
		},
		beforeSend	: function(data){
			showNoFormDialog("loading", "正在读取数据...");
			showNoFormDialog("loading", "正在读取数据...");
		}
	});
}
