...
function sx_main_height(){
	var main_left=$('.sx_manage .main_left'),
		main_right=$('.sx_manage .main_right'),
...
//$(pub_switch);
function pub_switch(){
	var tab=$('#pub_switch_tab li a'),		
		contain=$('#pub_switch_con .iterm_wrap');
		tab2=$('#pub_switch_tab2 li a')
		contain2=$('#pub_switch_con2 .iterm_wrap');
		tab3=$('#pub_switch_tab3 li a')
		contain3=$('#pub_switch_con3 .iterm_wrap');
	pub_switch();
	pub_switch2();
	pub_switch3();
	function pub_switch(){
		contain.hide();
		tab.parent().first().addClass('current');		
		tab.click(
			function(index){
	...
	function pub_switch2(){
		contain2.hide();
		tab2.parent().first().addClass('current');		
		tab2.click(
			function(index){
				$(this).parent().addClass('current').siblings().removeClass('current');
				var index=tab2.index(this);
				contain2.eq(index).show().siblings().hide();
				changepage(index);
				return false;
			}
		).first().click();		
	}
	function pub_switch3(){
		contain3.hide();
		tab3.parent().first().addClass('current');		
		tab3.click(
			function(index){
				$(this).parent().addClass('current').siblings().removeClass('current');
				var index=tab3.index(this);
				contain3.eq(index).show().siblings().hide();
				changepage(index);
				return false;
			}
		).first().click();		
	...
	function changepage(index){
		var pre=contain.eq(index).find('.page_up'),
			next=contain.eq(index).find('.page_down'),
			list=contain.eq(index).find('.list li'),				
			count=0,
			list_li_max=5,
			screen_num=list.length/list_li_max,
			prevelement = list.eq((screen_num-1)*list_li_max-1).prevAll(),
			nextelement = list.eq((screen_num-1)*list_li_max-1).nextAll();
		pre.addClass('disabled');
		if(list.length<list_li_max){
