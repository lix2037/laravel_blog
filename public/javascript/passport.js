		...
		if(checkEmail(user_name_))
		{
		    ...
		    $.ajax({
		        ...
		        success:function(msg){
		            if(msg.success == '1')
			        {
				        $('#li_user_name').addClass('correct');
			            $('#tip_user_name').html('<div class="tip"><i class="ico_correct"></i>该邮箱可注册</div>');
			            $("#get_verify_button").css("display","inline"); 
		            }else if(msg.success == '3')
			        {
		            	$('#li_user_name').addClass('error');
		            	$('#tip_user_name').html('<div class="tip"><i class="ico_error"></i>该邮箱已为注册用户，请重新输入或使用该邮箱直接<a href="/user/login">登录</a></div>');
		            	$("#get_verify_button").css("display","none"); 
		            }else if(msg.success == '4')
		            {
		            	$('#li_user_name').addClass('error');
		            	$('#tip_user_name').html('<div class="tip"><i class="ico_error"></i>该邮箱已为绑定邮箱，请重新输入</div>');
		            	$("#get_verify_button").css("display","none"); 
		            }else
		            {
			            alert(msg.message);
		...
		if(checkMobile(user_name_))
		{
		    ...
		    $.ajax({
		        ...
		        success:function(msg){
			        ...
			        {
				        $('#li_user_name').addClass('correct');
			            $('#tip_user_name').html('<div class="tip"><i class="ico_correct"></i>该手机号码可注册</div>');
			            $("#get_verify_button").css("display","inline"); 
		            }else if(msg.success == '3')
			        {
		            	$('#li_user_name').addClass('error');
		            	$('#tip_user_name').html('<div class="tip"><i class="ico_error"></i>该手机号码已为注册用户，请重新输入或使用该号码直接<a href="/user/login">登录</a></div>');
		            	$("#get_verify_button").css("display","none"); 
		            }else if(msg.success == '4')
		            {
		            	$('#li_user_name').addClass('error');
		            	$('#tip_user_name').html('<div class="tip"><i class="ico_error"></i>该手机号码已为绑定手机，请重新输入</div>');
		            	$("#get_verify_button").css("display","none"); 
		            }else
		            {
			            alert(msg.message);
		    ...
		    $.ajax({
		        ...
		        success:function(msg){
		            if(msg.success == '1')
			        {
		            	...
		            	$('#tip_verify').html('<div class="tip"><i class="ico_correct"></i></div>');
		            }else
		            {
		            	$('#li_verify').addClass('error');
		    			$('#tip_verify').html('<div class="tip"><i class="ico_error"></i>验证码不正确</div>');
		            }
		        },
		        error:function(){
		            alert('系统出错!');
...
function checkEmail(user_name_) {
	...
	$('#li_user_name').removeClass('error');
	$('#li_user_name').removeClass('correct');
	if (user_name_ == '') {
		$('#li_user_name').addClass('error');
		$('#tip_user_name').html('<div class="tip"><i class="ico_error"></i>请填写邮箱</div>');
		return false;
	} else if (!checkemail.test(user_name_)) {
		$('#li_user_name').addClass('error');
		$('#tip_user_name').html('<div class="tip"><i class="ico_error"></i>请填写正确的邮箱</div>');
		return false;
	}
	return true;
}
...
function checkMobile(user_name_) {
	...
	if (user_name_ == '') {
		$('#li_user_name').addClass('error');
		$('#tip_user_name').html('<div class="tip"><i class="ico_error"></i>请填写手机号码</div>');
		return false;
	} else if (!checkmobile.test(user_name_)) {
		$('#li_user_name').addClass('error');
		$('#tip_user_name').html('<div class="tip"><i class="ico_error"></i>请填写正确的手机号码</div>');
		return false;
	}
	return true;
}
...
function checkUsername(user_name_){
	...
	if (user_name_ == '') {
		$('#li_user_name').addClass('error');
		$('#tip_user_name').html('<div class="tip"><i class="ico_error"></i>请填写用户名(邮箱或手机号码)</div>');
		return false;
	} else if (!checkemail.test(user_name_)) {
		if(!checkmobile.test(user_name_)){
			$('#li_user_name').addClass('error');
			$('#tip_user_name').html('<div class="tip"><i class="ico_error"></i>请正确填写用户名(邮箱或手机号码)</div>');
			return false;
		}else {
			$('#li_user_name').addClass('correct');
	        $('#tip_user_name').html('<div class="tip"><i class="ico_correct"></i></div>');
			return true;
		}
	} else {
		$('#li_user_name').addClass('correct');
...
function checkPassword1(password_){
	...
	$('#normal_password').html('');
	$('#li_password').removeClass('error');
	$('#li_password').removeClass('correct');
	if (password_ == '') {
		...
		$('#li_password').addClass('error');
		$('#tip_password').html('<div class="tip"><i class="ico_error"></i>密码长度在6-16之间，只能包含字母、数字和下划线</div>');
		return false;
	}
	return true;
}
...
function checkPasswordconfirm() {
	$('#li_passwordconfirm').removeClass('error');
	$('#li_passwordconfirm').removeClass('correct');
	var passwordconfirm_ = $('#passwordconfirm').val();
	var password_ = $('#password').val();
	if (passwordconfirm_ == '') {
		$('#li_passwordconfirm').addClass('error');
		$('#tip_passwordconfirm').html('<div class="tip"><i class="ico_error"></i>请填写确认密码</div>');
		return false;
	} else if (passwordconfirm_ != password_) {
		$('#li_passwordconfirm').addClass('error');
		$('#tip_passwordconfirm').html('<div class="tip"><i class="ico_error"></i>两次密码输入不一致</div>');
		return false;
	} else {
		$('#li_passwordconfirm').addClass('correct');
		$('#tip_passwordconfirm').html('<div class="tip"><i class="ico_correct"></i></div>');
		return true;
	}
}
	...
	$.ajax({
		...
		success : function(msg) {
			if (msg.success == '1') {
				...
				alert(msg.message);
				$('#get_verify_button').html('<a href="javascript:void(0)" onclick="getRetrieveVerify();" class="btn">获取验证码</a>');
			}
		},
		error : function() {
			alert('系统出错!');
...
function checkResetPasswordForm(){
	...
	if(passwordconfirm_ == ''){
		$('#li_passwordconfirm').addClass('error');
		$('#tip_passwordconfirm').html('<div class="tip"><i class="ico_error"></i>请输入确认密码</div>');
		return false;
	}
